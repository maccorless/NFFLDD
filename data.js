  ...
  while (cur <= end) {
    ...
    if (dow !== 5 && dow !== 6) { // skip Fri/Sat
      dates.push({
        ...
        full: dayNames[dow] + ', ' + monthNames[cur.getMonth()] + ' ' + cur.getDate(),
      });
      i++;
    }
...
window.SEED_RESPONSES = {
  wags: Object.fromEntries(window.LEAGUES.wags.managers.map(m => [m.name, { note: '' }])),
  fex:  Object.fromEntries(window.LEAGUES.fex.managers.map(m => [m.name, { note: '' }])),
};
